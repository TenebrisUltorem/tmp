        ...
        .draw(|this, _, area, buf| {
            ...
            let view = match this.state() {
                InteractionState::Hovered => 
                    Paragraph::new(Line::from(LAST_TRACK_BUTTON_LABEL_HOVERED).centered())
                        .block(Block::bordered().border_type(BorderType::Thick)),
                InteractionState::Pressed => 
                    Paragraph::new(Line::from(LAST_TRACK_BUTTON_LABEL_PRESSED).centered())
                        .block(Block::bordered()),
                _ => 
                    Paragraph::new(Line::from(LAST_TRACK_BUTTON_LABEL).centered())
                        .block(Block::bordered())
            };
